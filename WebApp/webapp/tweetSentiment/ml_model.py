...
def prediction_model(tweet):
    loaded_vec = pickle.load(open(r'tweetSentiment/tfidf_mydataset.sav', "rb")) #load saved tfidf model
    tweet = loaded_vec.transform(np.array([tweet])) #apply tfidf model to tweets

    nb = pickle.load(open(r'tweetSentiment/finalized_model_mydataset.sav', 'rb')) #load saved algorithm
